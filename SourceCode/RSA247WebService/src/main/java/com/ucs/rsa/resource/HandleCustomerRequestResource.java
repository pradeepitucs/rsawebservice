...
public class HandleCustomerRequestResource {
	...
	@RequestMapping(value = "/updatecustomerissue", method = { RequestMethod.POST })
	private ModelAndView updateCustomerIssue(@RequestParam("issue_id") final int issue_id,
		...
		if (customerRequest != null) {
			...
			Runnable r = new Runnable() {
				public void run() {
					sendNotificationToCustomer(customerRequest.getCustomerModel().getUserId(),
							customerRequest.getIssueStatus(), emp_id, issue_id);
				}
			};
			new Thread(r).start();
		}
	...
	@RequestMapping(value = "/rejectcustomerissue", method = { RequestMethod.POST })
	private ModelAndView rejectCustomerIssue(@RequestParam("issue_id") final int issue_id,
			...
			@RequestParam("rejectComment") final String reject_comment, @RequestParam("employee_id") final int emp_id) {
		CustomerRequestModel customerRequestModel = new CustomerRequestModel();
		customerRequestModel.setIssueId(issue_id);
		customerRequestModel.setIssueStatus(issue_status);
	...
	@RequestMapping(value = "/updateEmployeeLocation", method = { RequestMethod.POST })
	...
	@RequestMapping(value = "/updatecustomerissuebyserviceprovider", method = { RequestMethod.POST })
	private ModelAndView updateCustomerIssueByEmployee(@RequestParam("issue_id") final int issue_id,
			...
			@RequestParam("issue_status") final String issue_status,
			@RequestParam("service_type") final String serviceType, @RequestParam("employee_id") final int emp_id,
		...
		CustomerRequestModel customerRequest = getCustomerRequestService().updateCustomerIssueByServiceProvider(
				customerRequestModel, customerSubIssueModelList, serviceProviderCommentModel);
	...
	private ModelAndView updateIssuelist(@RequestParam("issue_id") final int issue_id,
		...
		if (customerRequestResponse.get(1).equalsIgnoreCase("Accepted")) {
			Runnable r = new Runnable() {
				public void run() {
					sendNotificationToCustomer(Integer.parseInt(customerRequestResponse.get(0)),
							customerRequestResponse.get(1), emp_id, issue_id);
				}
			};
			new Thread(r).start();
		}
		...
		if (serviceCompleted != null) {
			Runnable r = new Runnable() {
				public void run() {
					CustomerRequestModel customerRequestModel = new CustomerRequestModel();
					customerRequestModel = getCustomerRequestService().get(CustomerRequestModel.class, issue_id);
					sendNotificationToCustomer(customerRequestModel.getCustomerModel().getUserId(), issue_status,
							emp_id, issue_id);
				}
			};
			new Thread(r).start();
	...
	private void sendNotificationToCustomer(int customerId, String status, int employeeID, int issueID) {
		...
		servicetype= getCustomerRequestService().get(ServiceTypeModel.class, customerRequestmodel.getServiceTypeModel().getServiceTypeId());
		EmployeeModel employeeModel = new EmployeeModel();
		...
		String smsForCustomer = "Your Service request for a "+ servicetype.getServiceType() +" has been accepted by "+serviceProviderModel.getServiceProviderName() +", mobile no. +91 "+ employeeModel.getMobileNo()+".";
		String smsForEmployee = "You have accepted a request for "+ servicetype.getServiceType() +" of Mr."+ customerModel.getFirstName() +" " + customerModel.getLastName() + ". Mobile no. +91 "+ customerModel.getMobileNo() +". Please start as soon as possible.";
		SmsLane.SMSSender("pradeepit", "pradeep143", "91" + customerModel.getMobileNo(), smsForCustomer, "WebSMS",
				"0");
	...
	private ArrayList<Double> getServiceProviderLocationFromServiceProviderId(int serviceProviderID) {
		ArrayList<Double> location = getCustomerRequestService()
